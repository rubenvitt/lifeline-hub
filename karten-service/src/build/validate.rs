...
mod tests {
    ...
    fn bremen_bounds_liegen_in_de_box() {
        assert!(bounds_passen(
            Some((8.48, 53.01, 8.99, 53.23)),
            erwartete_box("DE").unwrap()
        ));
    }
    #[test]
    ...
    fn jede_baubare_region_hat_eine_box() {
        ...
        for r in crate::regions::alle() {
            assert!(
                erwartete_box(r.region).is_some(),
                "keine erwartete_box für {} ({})",
                r.region,
                r.slug
            );
        }
    ...
    fn laender_bounds_passen_in_ihre_metropolitane_box() {
        ...
        for (code, b) in faelle {
            assert!(
                bounds_passen(Some(*b), erwartete_box(code).unwrap()),
                "{code} bounds passen nicht"
            );
        }
    }
    ...
    fn kompakte_boxen_weisen_uebersee_ab_welt_ist_global() {
        ...
        assert!(
            !bounds_passen(
                Some((-68.4, 12.0, -68.2, 12.3)),
                erwartete_box("NL").unwrap()
            ),
            "Bonaire ≠ NL"
        );
        assert!(
            !bounds_passen(
                Some((55.2, -21.4, 55.9, -20.9)),
                erwartete_box("FR").unwrap()
            ),
            "Réunion ≠ metrop. FR"
        );
        // Welt bleibt bewusst global — jedes plausible Planet-Extent passt.
        assert_eq!(
            erwartete_box("WORLD").unwrap(),
            (-180.0, -86.0, 180.0, 86.0)
        );
        assert!(bounds_passen(
            Some((-179.0, -84.0, 179.0, 84.0)),
            erwartete_box("WORLD").unwrap()
        ));
    }
}
