...
pub async fn tick(st: &WorkerState) -> bool {
    let Some(_guard) = st.registry.try_lock_build() else {
        return false;
    };
    let Some((id, slug)) = st.registry.naechster_queued() else {
        return false;
    };
    match regions::finde(&slug) {
        ...
        None => st
            .registry
            .set_status(id, JobStatus::Failed(format!("unbekannter slug {slug}"))),
    }
    true
}
...
mod tests {
    use super::*;
    use crate::{
        build::{BuildArtefakt, BuildRunner},
        jobs::Registry,
    ...
    async fn tick_faehrt_queued_job() {
        ...
        let st = WorkerState {
            registry: reg.clone(),
            runner: Arc::new(OkRunner(f.path().into())),
