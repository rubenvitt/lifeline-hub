...
| L‑2 | **Taktische Gliederung** | Einheiten/Abschnitte (K&M‑3) als taktische Zeichen (DV 102); operativer Fahrzeug-/Kräfte-Standort (FMS) | L‑1 + K&M‑3 | ✅ **DONE** — Plan `docs/superpowers/plans/2026-05-31-lage-taktische-gliederung.md`, Branch `feat/l2-taktische-gliederung` (Migr. `0035`; Geo-/Symbol-Spalten + `tz_organisation`; Geo-PATCH-Routen + SSE-Streams + Führungskräfte-Lesepfad; `GET/PATCH /api/organisation`; FE: `taktische-zeichen-react` DV‑102-Marker, `terra-draw` Abschnitts-Polygone, Inspector/Sidebar/Org-Tab); subagent-getrieben umgesetzt |
| L‑3 | **Gefahren- & Absperrzonen** | Freies Zeichnen von Flächen/Linien, entitätsloser Annotations-Layer (neue `lage_zone`-Entity, Typ-Katalog, ETB-Spur) | L‑1 | ✅ **DONE** — Plan `docs/superpowers/plans/2026-06-01-lage-gefahren-absperrzonen.md`, Branch `feat/lage-gefahren-absperrzonen` (Migr. `0036` `lage_zone`; `src/lage_zone/` + `src/routes/lage_zone.rs`: CRUD + 422-Validierung der Typ↔Geometrie-Bindung + automatische ETB-Spur (eingerichtet/geändert/aufgehoben) + neuer SSE-Kanal `lage_zone`; FE: `zonenStil.ts` (typabgeleiteter Stil), generisches `zeichnen.ts` (Polygon+Linie), Zonen-Layer/-Inspector/-Live in `LagekartePage`); subagent-getrieben umgesetzt. **Damit ist Teilprojekt 4 funktional abgeschlossen** (L‑1/L‑2/L‑3); L‑4 bleibt geparkt |
...
**Spec L‑2:** `docs/superpowers/specs/2026-05-31-lage-taktische-gliederung-design.md`
**Spec L‑3:** `docs/superpowers/specs/2026-06-01-lage-gefahren-absperrzonen-design.md`
