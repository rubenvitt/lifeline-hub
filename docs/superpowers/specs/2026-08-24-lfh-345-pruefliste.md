...
| --- | --- | --- |
| Speicherpfade mit persistentem Fehlerzustand | **0 von 7** (alle nur Toast) | **7 von 7** |
...
| Anmelden-Knopf vollständig sichtbar nach | **1,06 s** | **0,35 s** |
...
| 6 · Kein Status allein über Farbe | **erfüllt** | Die Fehlermarke an der Zeile trägt **zwei** Kanäle: den Rand (Farbe) und die Meldung im Kopf der Seite, die den Fehlertext im Klartext nennt. Der Rechte-Hinweis ist reiner Text mit Icon. Der Einsatz-Status steht als **Wort** statt als Wire-Wert |
...
| 13 · Fokus nie verdeckt | **erfüllt** | Die neue sticky Speicherleiste der Einstellungs-Sektionen ist die einzige verankerte Fläche und wird gegen `e2e/fokus-verdeckung.spec.ts` gehalten |
