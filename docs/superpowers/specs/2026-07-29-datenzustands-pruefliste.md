...
| 7  | **erfüllt** | Rot bedient nichts: `SeitenFehler` ist `type="error"` (Ausfall), `SeitenStandVeraltet` bewusst `type="warning"` — dort sind Daten **da**, die Zeilen darunter sind echt, nur alt. Das Overlay der Lagekarte ist dagegen `type="error"` (`LagekartePage.tsx:292`), und das ist die richtige Stufe: dort fehlen Objekte **ganz**, und eine Karte ohne Objekt ist von einer Lage ohne Objekt nicht zu unterscheiden — ein unsichtbarer Ausfall, kein Hinweis. Die Regel bleibt gewahrt, weil das Overlay **nichts bedient**: ein `banner`-Alert mit `showIcon` als zweitem Kanal und ohne jeden Knopf (von `LagekartePage.test.tsx` gepinnt) — es gibt kein rotes Bedienelement. |
...
| 14 | **erfüllt** | Kopfzeile, Identifierspalte und Spaltenschalter kommen unverändert aus `KatalogTabelle` (B1/B2); keine Tabelle wurde in Karten aufgelöst. **Korrektur gegenüber der ersten Fassung dieser Zeile:** das Primitiv wurde doch angefasst — der Review fand, dass die in D4 behauptete zentrale Ladeunterdrückung dort gar nicht existierte (`loading`/`locale` fielen durch `...rest` an antds `Table` durch, der Leertext stand während des Ladens im DOM). Das ist **kein** Verstoß gegen D3: D3 verbietet ein *Fehler*-Prop am Tabellen-Primitiv, D4 verlangt die *Lade*unterdrückung ausdrücklich dort. Die Guard-Invarianten (Inventar, Bildlauf, Blätterung, Ordnung, Schließung) wurden nachgeprüft und sind unberührt. |
