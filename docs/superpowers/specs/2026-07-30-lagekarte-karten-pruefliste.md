...
Grep — die einzeilige Regex sah in `SnapshotLeiste.tsx` **keine einzige** der sechs Stellen):
...
| **2, 12, 13** | Layout-Nachweise, die jsdom nicht führen kann: Höhe im Handschuh, Zeitachse bei ~390 px, Fokus hinter den schwebenden Aufbauten | **noch nicht getickt** — es wäre der erste Dichte-e2e-Nachweis des Repos und damit eine eigene Entscheidung (identisch zu LFH-365) |
| **14** | Spaltenschalter mit Zähler fehlt (`KatalogTabelle` statt `Datensicht`) | **noch nicht getickt** — Bestand, dieselbe Lage wie beim ETB |
