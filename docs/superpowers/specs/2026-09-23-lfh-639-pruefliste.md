...
| Stand | Commit `e8b357d1` auf `claude/lfh-639-048ca8` |
...
| Nicht enthalten | Lagekarte, Personenverknüpfung, Offline-Erfassung, Verlaufsansicht der Meldereihen (Non-Goals aus design.md, Nachzüge unter Aufgabe 5.3) |
...
Geschlossenes trägt **[abgeleitet]**. Wo das Zielticket noch nicht angelegt ist, steht
„→ Ticket folgt: …" mit dem vorgeschlagenen Titel.

## Die Nachweise

| Nachweis | Ergebnis | Stand |
| --- | --- | --- |
| Gate 3, `e2e/gate3-trefflaeche.spec.ts` „Betreuung: Karten- und Zeilenaktionen folgen der Dichte-Staffel 30 / 48 / 72 px", 1366 px, zwei gesäte Bezirke, drei Stellen (eine geschlossen) | „Stand melden" **30 / 48 / 72** px (2 Knoten) · Dreipunkt Bezirk **30 / 48 / 72** (2) · „Belegung melden" **30 / 48 / 72** (2) · Dreipunkt Stelle **30 / 48 / 72** (3) · Kopfaktion **30 / 48 / 72** (1) · „Betreuungsstelle anlegen" **30 / 48 / 72** (1) · Abstand Karte **15 / 23 / 33** px · Abstand Tabellenzeile **11 / 18 / 26** px | `e8b357d1` |
...
| Grep über `betreuung/` (ohne Tests) und `pages/BetreuungPage.tsx` | Farbliterale **0** · `animation`/`blink`/`keyframes`/`transition` **0** · `sticky`/`fixed` **0** · `danger` **1** (`okButtonProps` des Storno-Dialogs) · `size=` **1** (`Space size="middle"`, Abstandsmaß, nicht interaktiv) · `zufluss` nur im Kommentar (beide Blöcke laufen mit der Vorgabe `sammelbanner`) | `e8b357d1` [abgeleitet] |
...
| 3 | **Rückmeldung vor der Serverantwort** ≤ 100 ms | **erfüllt** | Dialogknöpfe laufen mit `loading` (`laeuft` an `ErfassungsModal`), die Blöcke zeigen beim Laden die Ladeanzeige der `Datensicht` statt eines Leerzustands. `BetreuungPage.test.tsx` „ein Abruffehler ist kein Leerzustand"; `BetreuungDialoge.test.tsx` „eine Ablehnung (422) lässt die Felder stehen und zeigt den Grund im Dialog" | — |
...
**Bilanz:** 9 erfüllt · 4 offen · 2 nicht anwendbar.
...
| 13 | Fokus nie verdeckt | **offen** | Nicht gemessen; im Handschuh-Betrieb bei 390 px kann ein Dialog höher als der Schirm sein. Muster: `e2e/dokumente.spec.ts` „Fokus nie verdeckt im Ablegen-Dialog …" | → Ticket folgt: „Betreuung: Browser-Nachweis Kontrast und Fokus-Verdeckung (Prüfliste LFH-639, Zeilen 5 und 13)" |
...
| — | Sichtbarkeit | **erfüllt** | Ohne sichtbares Modul keine Anfrage: `useModulZaehler.test.ts` „lädt den Betreuungszähler nur bei sichtbarem Modul (LFH-639)"; `useEvakuierungKennzahl.test.tsx` „lädt NICHT bei ausgeblendetem Modul — kein Abruf, Zustand `aus` (weder fehler noch null)", „lädt NICHT bei rollen-gesperrtem Modul". Live-Ereignis `betreuung` nur an Modul-Leser: `tests/betreuung.rs` `live_ereignis_nur_an_leser_mit_modulrecht` | — |
...
| --- | --- | --- |
| T1-5, T1-13, T2-13 | Kontrast und Fokus-Verdeckung im Browser nicht gemessen | → Ticket folgt: „Betreuung: Browser-Nachweis Kontrast und Fokus-Verdeckung (Prüfliste LFH-639, Zeilen 5 und 13)" |
| T1-8 | kein Helligkeitsregler in der Anwendung | LFH-397, app-weit |
| T1-9 | volle und überbelegte Stellen nur in der Tabellenzeile, Tabelle ggf. unter der Falz | → Ticket folgt: „Betreuung: volle und überbelegte Stellen im Blickfeld" |
