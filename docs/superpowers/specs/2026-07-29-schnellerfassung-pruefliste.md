...
| 12a · Nachtrag aus dem Review | **behoben** | Der adversarische Review fand drei Dinge, die alle Gates passiert hatten. **(a)** `ErfassungsModal` reichte `onCancel` roh durch — Escape, Schließkreuz und Maskenklick setzten also nicht zurück, obwohl acht Aufrufer ihren eigenen Reset im Vertrauen darauf gelöscht hatten; `destroyOnHidden` fängt das nicht, der Speicher von rc-field-form überlebt. Gemessen an `PersonalFormModal`: Person bearbeiten, Escape, „Person anlegen" — das Formular trug Name, Personalnummer und Telefon der bearbeiteten Person, Speichern legte sie als Dublette an. **(b)** Die Serien-Marke wurde nur in `onFinish` verbraucht; eine gescheiterte Pflichtfeld-Prüfung ließ sie stehen und färbte das nächste reguläre Absenden zum Serienlauf. **(c)** Der neue Anlegedialog schickte die Alarmzeit als lokale Wanduhrzeit in ein UTC-Feld — jeder neue Einsatz trug eine um den Zonenversatz verschobene Alarmzeit. Alle drei behoben, alle drei mit einer Mutationsprobe belegt (Fix zurückgedreht → Test rot) |
