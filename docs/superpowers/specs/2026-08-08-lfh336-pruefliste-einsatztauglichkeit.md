...
|---|---|---|---|
| 1 | **1 · Treffläche** — Boden ≥ 24 × 24 CSS px oder 24-px-Umkreis frei (WCAG 2.5.8 AA); zeitkritische Aktion ≥ 48 × 48 px mit ≥ 8 px Abstand (WCAG 2.5.5 / Material 48 dp). | **erfüllt** — `.lfh-kz`, `.lfh-kachel__mehr`, `.lfh-knopf` und die neuen Kurzlisten-Zeilen (`a.lfh-zeile`) lesen alle `min-height: var(--lfh-zeilenhoehe)` (kompakt 30 px ≥ 24-px-Boden), belegt `theme/sprache.css:207-222` (`.lfh-kz`), `:320-330` (`.lfh-kachel__mehr`), `:447-455` (`.lfh-zeile`), `:488-491` (`a.lfh-zeile`) und `:603-608` (`.lfh-knopf`), sowie Quellpin-Test `LageDashboardPage.test.tsx` „keine harte min-height mehr in sprache.css — die Hoehen lesen die Staffel" (Zeile 667). `size="small"` = 0 Treffer (`grep -c 'size="small"' LageDashboardPage.tsx` → 0). | **erfüllt** — kein neues `size="small"` (`grep -c` → 0), kein `controlHeight` außerhalb der Theme-Schicht; Button/Input/Card-Link erben die Steuerhöhe vom `ConfigProvider`. Die Karte selbst (`KACHEL_MIN_HOEHE = 120`, `EinsaetzePage.tsx:66,193,296`) überschreitet in jeder Dichtestufe (30/48/72) jeden geforderten Boden deutlich. |
| 2 | **2 · Handschuh-Modus** vorhanden und geprüft — Zeilenhöhe ≥ 72 px (= 19,05 mm [abgeleitet], MIL-STD-1472F Fig. 12), Abstand ≥ 16 px [abgeleitet aus Fig. 24, last contact]. | **teilweise erfüllt** — der Mechanismus ist vorhanden: `--lfh-zeilenhoehe` steht in der Handschuh-Stufe auf 72 px (`theme/rollen.css:142`, gegen `tokens.ts` gepinnt in `rollen.guard.test.ts:165`), und dieselben Selektoren wie bei Kriterium 1 lesen ihn. **Geprüft im Browser ist es für diese Route nicht**: `e2e/dichte.spec.ts` misst ausdrücklich nur den Anmelde-Knopf auf `/login` und grenzt sich explizit ab („die vollständige Trefflächen-Geometrie … bleibt bei den Modulpaketen"), `e2e/trefflaeche-tablet.spec.ts` deckt `/einsaetze/:id/personal`. Für `/einsaetze/:id/lage-dashboard` existiert kein `boundingBox()`-Test in Handschuh-Dichte. → offen → **LFH-396** (<https://app.clickup.com/t/86cb2q96u>) | **teilweise erfüllt** — gleicher Mechanismus (ConfigProvider-Dichte), gleiche Lücke: kein Gate-3-Browser-Test für `/einsaetze` in Handschuh-Dichte. `e2e/gate1-ueberlauf.spec.ts` deckt die Route nur für Breitenüberlauf (Gate 1), nicht für Trefflächenhöhe (Gate 3). → offen → **LFH-396** (<https://app.clickup.com/t/86cb2q96u>) |
