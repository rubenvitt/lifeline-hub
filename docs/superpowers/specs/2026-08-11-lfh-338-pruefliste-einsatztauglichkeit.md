...
| 2 | **Handschuh-Modus** vorhanden und geprüft — Zeilenhöhe ≥ 72 px, Abstand ≥ 16 px. | **offen → LFH-515** (<https://app.clickup.com/t/123zgec121q>) | Der Mechanismus greift (alle neuen Elemente hängen am `ConfigProvider`, keine punktuelle Größe), im Browser ist er für **diese** Route aber nicht gemessen. `e2e/meldebild-tabelle.spec.ts` setzt die Handschuh-Dichte an genau einer Stelle — für die Trefffläche des antd-Aufklapp-Symbols, und dort ausdrücklich **protokollierend statt zusichernd**. Für `Segmented`, Filter-Marken und Verdichtungszeile ist die 72-px-Stufe unbelegt. Dieselbe Lücke wie bei LFH-336 (LFH-396) und LFH-337 (LFH-516). **Korrektur (05.09.2026, beim Schließen von LFH-396):** eine frühere Fassung verwies diese Zeile auf LFH-396 — das Ticket ist auf Lage-Dashboard und Einsatzauswahl gescopt, sein Spec deckt keine Fläche der Kräfteübersicht. Die Flächen von C3 liegen als eigener Nachzug in LFH-515. |
