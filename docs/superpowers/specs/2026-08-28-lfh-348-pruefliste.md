...
| **9 · Kritische Anzeigen im Blickfeld** | **erfüllt** (N2 eingelöst, LFH-494) | Der Erfolg bleibt still (EEMUA 191, C7); der **Autosave-Fehler** steht seit LFH-494 als `SpeicherFehler` im Kopf beider Detailseiten, nicht mehr als Toast. Der Grund fällt erst beim nächsten **gelungenen** Speichern — nicht beim nächsten Versuch, sonst blinkte er bei stehendem 503 im 30-s-Takt. Vorher stand „ungespeicherte Änderungen" ohne den Grund da: das WAS ohne das WARUM |
...
| **9 · Kritische Anzeigen im Blickfeld** | **erfüllt** (N2 eingelöst, LFH-494) | Das Modal bleibt bei Ablehnung offen und behält die Werte (die Hülle leert nur bei Erfolg, `mutateAsync`); seit LFH-494 steht der **Grund** als `SpeicherFehler` über den Feldern. Dieser Pfad räumt weiter beim Absenden (`mutation.error`) — hier drückt ein Mensch den Knopf, es gibt keinen Auto-Retry wie beim Autosave |
...
| **N2 → LFH-494** ✅ **eingelöst 11.09.2026** | Speicher-/Anlegefehler als `SpeicherFehler` in der Seite bzw. im Dialog statt nur als Toast (Fortschreibung von C10/H14 auf die Entwurfsseiten). `useEntwurfVerlustschutz` hat `onFehler` **verloren** und liefert stattdessen `speicherFehler`/`meldeSpeicherfehler` | beide Detailseiten, `LageberichtePage.tsx` |
...
| **N5 → LFH-535** | Aufgefallen im Review zu LFH-494, dort bewusst nicht mitgefixt: `freigebenMutation` meldet weiter nur per Toast, obwohl antd das Bestätigungs-Modal bei rejectendem `onOk` **offen** hält — der einzige Grund geht nach ~3 s. Kein Regress (Bestand) und formal ausserhalb der N2-Grenze, weil eine Freigabe ein **Zustandsübergang** ist und kein Speicherpfad. Ob ein gescheiterter Übergang denselben Träger bekommt, ist eine Bedienentscheidung — Kandidat ist die in LFH-494 gebaute Bauform „Grund im Modal-Body" (`EntwurfNavigationSchutz`). Gilt für beide Zwillingsseiten gemeinsam; `fortschreibenMutation` ist nicht betroffen (kein Dialog) | `BefehlDetailPage.tsx`, `LageberichtDetailPage.tsx` |

Je ein Ticket; N1, N2 und N5 betreffen beide Zwillingsseiten und gehören
