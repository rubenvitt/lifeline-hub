...
| **7 · Eine Farbe = eine Bedeutung** | **teilweise erfüllt** | Rot bedient nichts; kein `danger` auf der Seite. **Bestand, nicht angefasst:** `<Tag color="green">` für „Freigegeben" (`:228`) ist ein antd-Preset am Wire-Status, kein `StatusTag` mit Vertragstyp — derselbe Befund wie N4 in C12, hier am Berichtsstatus. Nicht in C13 behoben, weil `LAGEBERICHT_STATUS` (`kommunikation/`) bewusst auf der Phasenachse außerhalb des A2-Vertrags liegt und der Wechsel eine Entscheidung für alle vier Kommunikationsmodule wäre → **Nachzug N1** |
...
| **7 · Eine Farbe = eine Bedeutung** | **teilweise erfüllt** | Rot bedient nichts. Der Berichtsstatus läuft über die Phasenachse (`LAGEBERICHT_STATUS`), bewusst außerhalb des A2-Vertrags — derselbe Punkt wie Tabelle 1 Zeile 7 → **Nachzug N1** |
