...
| 15 | **Erfassungsmaske vollständig** | **erfüllt** | `ErfassungsModal`: Absendeknopf im `<form>`, keine Modal-Fußzeile (Vitest); Anfangsfokus auf „Datei wählen“ (e2e); Tab-Reihenfolge ohne zweiten Stopp (e2e); Serienmodus hält offen und leert (Vitest); **nach „Speichern und nächste“ steht der Fokus wieder auf „Datei wählen“ (e2e, Code-Review C2)** — die Hülle nimmt das ausdrückliche Fokusziel `[data-erfassung-fokus]` statt des versteckten Datei-Inputs; Probe: Serien-Fokus der Hülle neutralisiert → genau dieser Schritt rot; Reset auf allen Wegen hinaus trägt die Hülle [abgeleitet, aus der Hülle `components/Erfassung.tsx` und ihren Tests, hier nicht gemessen]; `mutateAsync` lässt die Auswahl bei Ablehnung stehen (Vitest); Strg/⌘ + Enter = „Speichern und nächste“ [abgeleitet, Hülle] | — |
