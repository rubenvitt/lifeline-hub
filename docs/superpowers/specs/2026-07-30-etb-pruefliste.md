...
| **2, 12, 13** | Layout-Nachweise, die jsdom nicht führen kann: Zeilenhöhe im Handschuh, Chip-Leiste bei ~390 px, Fokus hinter dem angepinnten Kopf | **noch nicht getickt** — es wäre der erste Dichte-e2e-Nachweis des Repos und damit eine eigene Entscheidung |
| **14** | Spaltenschalter mit Zähler fehlt (`KatalogTabelle` statt `Datensicht`) | **noch nicht getickt** — Bestand, nicht von B5e verursacht |
| **5** | vier hartkodierte Farbwerte im angepinnten Erfassungskopf | **noch nicht getickt** — fällt mit dem nächsten Zugriff auf `.etb-erfassung-sticky` |

Die drei fett markierten Zeilen zeigen auf **kein** Ticket, und das steht hier ausdrücklich so
statt als „→ Zielticket". Ein Verweis auf eine Nummer, die es nicht gibt, ist schlechter als ein
offen benannter Rest: er liest sich wie erledigte Planung.
