...
| 13 · Fokus nie verdeckt | **erfüllt — der Nachzug LFH-465 ist eingelöst** | C8 führt **kein** neues `position: sticky` ein. Der AK-Halbsatz „unterhalb des Tablet-Breakpoints die Aktionsleiste am unteren Rand verankern" ist **bewusst nicht** umgesetzt: der gemessene Befund M73 („Freigeben rutscht aus dem sichtbaren Bereich") war ein **waagerechtes** Problem — ein `<Space>` ohne `wrap` — und ist mit dem Umbruch behoben. Eine sticky Aktionsleiste auf einer Seite mit langem Markdown-Formular ist dagegen genau die Konstruktion, auf die WCAG 2.4.11 zielt: das unterste fokussierte Feld läge dahinter. Sie bräuchte einen Verdeckungsnachweis nach dem Muster von `e2e/fokus-verdeckung.spec.ts` und ist damit ein eigener Vorgang → **das war LFH-465, und der ist erledigt** (09.09.2026): die Leiste steht unterhalb von `lg`, der Nachweis liegt in `e2e/befehl-aktionsleiste.spec.ts`, der Messkern ist nach `e2e/fokus-kern.ts` gezogen und wird von beiden Dateien geteilt. **Der Kern allein reichte nicht** — er meldet nur VOLLSTÄNDIGE Verdeckung, und ein 164 px hohes Abschnittsfeld ist von einer 78 px hohen Leiste nie vollständig verdeckt; die tragende Zusicherung ist deshalb der gemessene Fokus-Scroll-Abzug (`scroll-padding-block-end` am Scrollport): ohne ihn beginnt `einsatzunterstuetzung` beim Tabulatorwechsel **einen Pixel unterhalb** der Leistenoberkante, mit ihm bleiben 42 px frei (`kompakt`, Leiste 78 px) bzw. 133 px (`handschuh`, Leiste 184 px). Vier Mutationsproben belegen die Zeilen einzeln. `e2e/fokus-verdeckung.spec.ts` ist mit den C8-Änderungen grün (3/3) |
...
**0 Zeilen ohne Verdikt.** Stand 09.09.2026, nachdem LFH-465 Zeile 13 eingelöst hat:
...
| Nummer | Inhalt |
| --- | --- |
| **LFH-465** | ~~Verankerte Aktionsleiste am Befehlsentwurf unterhalb des Tablet-Breakpoints, samt Verdeckungsnachweis nach dem Muster von `e2e/fokus-verdeckung.spec.ts` (Prüflisten-Zeile 13)~~ — **erledigt 09.09.2026**, s. Zeile 13 oben |
