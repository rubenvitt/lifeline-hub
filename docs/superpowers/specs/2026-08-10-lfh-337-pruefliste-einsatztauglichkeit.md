...
Zeilen 378–412) — Nummerierung und Wortlaut sind dort übernommen, nicht neu erfunden.
Form nach dem Präzedenzfall `docs/superpowers/specs/2026-08-08-lfh336-pruefliste-einsatztauglichkeit.md`.
...
| 5 | **Kontrast in beiden Modi** — Text Tag ≥ 7 : 1, Nacht ≥ 5 : 1, nie < 4,5 : 1; Zustände, Rahmen, Fokusring ≥ 3 : 1. | **erfüllt** | Alle Farben kommen aus `theme/tokens.ts` (`farbenDunkel.*`), keine neue Hex-Farbe eingeführt (`grep -rniE '#([0-9a-f]{3}){1,2}\b'` über die fünf Kerndateien liefert nur `#fff` am Logo-Link/freien Verwaltungslink und `#001529` in Kommentaren). Der gesperrte Link nutzt `farbenDunkel.schwach` = 5,30 : 1 gegen den Kopfzeilengrund `#001529` (Rechnung unten bei AK6/Nachweis) — über dem geforderten 4,5 : 1. Der Rail-Aktivzustand nutzt `farbenDunkel.bedien` (8,67 : 1, aus LFH-328/A2 gemessen, unverändert weiterverwendet, in `IconRail.test.tsx` per Rollenvergleich gepinnt). Die Kopfzeile trägt in beiden Farbmodi denselben dunklen Grund (`farbenDunkel`, nicht der moduswechselnde Token), deshalb ist „beide Modi" hier bereits durch die Rollenwahl erfüllt. |
...
| 8 | **Helligkeits-/Kontrastregler** vorhanden und bei aktiver Warnung nicht bis AUS dimmbar. | **offen → LFH-397** (<https://app.clickup.com/t/86cb2qcq9>) | App-weite, bereits unter LFH-336 dokumentierte Lücke — kein Regler existiert irgendwo in der Anwendung. Nicht seitenspezifisch und nicht im Scope von LFH-337; dasselbe Ticket trägt sie bereits. |
...
| 10 | **Alarmbudget eingehalten** — 1–2 je 10 min im Dauerbetrieb, ≤ 3 Eskalationsstufen. | **nicht anwendbar** | Der Navigationsrahmen erzeugt keine Alarme. |
| 11 | **Warnverhalten** — kein Blinken auf lesbarem Text, jede Warnung quittierbar, jeder Ton mit visueller Entsprechung. | **erfüllt** | `grep -rn "animation\|blink\|@keyframes"` über die fünf Kerndateien liefert 0 Treffer — kein Blinken, keine Animation auf lesbarem Text. |
| 12 | **Kein Sprung unter dem Cursor** — CLS ≤ 0,1; neue Datensätze nur als opt-in-Sammelbanner. | **nicht anwendbar** | Kein SSE-getriebenes Nachladen verändert Reihenfolge oder Position von Rail-, Panel- oder Kopfzeilen-Elementen, während sie sichtbar sind. Die „Zuletzt"-Liste liest `localStorage` nur beim Mounten von `EinsatzLayout` (Navigationswechsel), nicht live während des Lesens; ein Wechsel entsteht ausschließlich durch die eigene Klickaktion der Person, nicht durch ein externes Ereignis unter dem Cursor. Die (bestehenden, nicht in diesem Branch geänderten) WIP-Zähler im Modul-Panel aktualisieren einen Zahlenwert in fester Zeilenposition, keine Layoutverschiebung. |
...
| 14 | **Tabellenseite vollständig** — fixierte Kopfzeile, fixierte Identifierspalte, umschaltbarer Spaltensatz, keine Auflösung in Karten. | **nicht anwendbar** | Keine `<Table>` im Navigationsrahmen (`grep -rn "<Table"` über die fünf Kerndateien → 0 Treffer; ein loses `grep "Table"` liefert zwei Fehltreffer aus „Führungs-**Tablet**" in Kommentaren, kein Komponenten-Treffer). |
| 15 | **Erfassungsmaske vollständig** — Defaults vorbelegt, „Speichern und nächsten anlegen", Sammelliste, Labels über dem Feld, volle Tastaturbedienung. | **nicht anwendbar** | Keine Erfassungsmaske im Navigationsrahmen; das Suchfeld der Befehlspalette ist ein Filter, kein Datenerfassungsformular. |

**Verteilung:** 5 erfüllt (1, 5, 7, 11, 13), 2 offen (2, 8 — beide mit bestehendem
Zielticket), 8 nicht anwendbar (3, 4, 6, 9, 10, 12, 14, 15).
...
Kriterien bleibt eine echte Lücke (Kriterium 2: kein Browser-Test in Handschuh-Dichte).
