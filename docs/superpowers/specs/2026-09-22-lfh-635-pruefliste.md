...
| Nicht enthalten | Kontrast-Messung (→ LFH-646) |
| Nachzug | Sammelbanner für fremde Neuzugänge: LFH-647, Kriterium 12 (Stand siehe dort) |

| Fläche | Stellvertreter | Browser-Messung |
| --- | --- | --- |
| **1 · Schichtliste mit Kopf und Vorgaben-Paneel** | `pages/AbloesungPage.tsx`, `abloesung/AbloesungKarte.tsx` | ja: Gate 1 (Überlauf), Gate 3 (Trefffläche, Abstand) |
| **2 · Erfassungsdialoge** (Schicht beginnen, Vollzug, Ablöser planen, Rhythmus) | `abloesung/AbloesungDialoge.tsx` | nein (Vitest) |
...
| 5 | **Kontrast in beiden Modi** — Text Tag ≥ 7:1, Nacht ≥ 5:1; Zustände ≥ 3:1 | **offen** | Entlastend, aber nicht gemessen: keine Farbliterale (Grep), Etikett über `StatusTag` (Rolle am Rand, `colorText` als Schrift, LFH-446), Alarmfläche aus `rollen.alarmFlaeche`. Einen Kontrast-Spec für die Route gibt es nicht | LFH-646 |
...
Nur die Zeilen, die sich von Tabelle 1 unterscheiden. Nr. 5, 7, 8 und 11 gelten wie dort.
