...
| Nachzüge | Kontrast-Messung mit LFH-646 (`db22648b`, Kriterium 5) · Sammelbanner für fremde Neuzugänge mit LFH-647 (Kriterium 12) |
...
| 5 | **Kontrast in beiden Modi** — Text Tag ≥ 7:1, Nacht ≥ 5:1; Zustände ≥ 3:1 | **offen** | Gemessen mit `e2e/abloesung-kontrast.spec.ts` (Werte unter „Die Nachweise“). Offen ist das Verdikt allein wegen der zwei app-weiten Rollen unten, nicht wegen der Seite. Jeder Text der Seite, der nicht unter eine der zwei app-weiten Ausnahmen fällt, hält Tag ≥ 7 (Minimum 7,05, auf der Karte 7,31, auch auf `alarmFlaeche`) und Nacht ≥ 5 (Minimum 6,19). Zustandsrand (überfällig, Vorwarnung) ≥ 3 gegen Seitengrund **und** Kartenfläche (Minimum 5,52), Etikettrand ≥ 3 gegen beide Flächen (Minimum 4,72). **Der Rand der planmäßigen und der abgelösten Karte ist nicht anwendbar:** er trägt die Linienfarbe (1,20–1,31), weil planmäßig kein Zustand ist. Der Spec sichert zu, dass er keinem Zustandsrand gleicht. **Ausnahmen, beide nicht seitenspezifisch:** Tertiärtext `schwach` (Augenbrauen, Sekundärtext samt „Stand“, Feldhilfe, Platzhalter, Ortspfad im Seitenkopf) liegt am Tag bei 5,20–6,37 und nachts bis 4,81 → LFH-643. Weiß auf `bedien` im Primärknopf liegt am Tag bei 6,59 → LFH-661. Beide liegen unter der Schranke dieser Zeile. Der Spec prüft sie gegen die Untergrenze 4,5 und hält sie so fest; die Ausnahme fällt, sobald die Tickets landen. Ein lokaler Farbwechsel auf der Karte änderte das Verdikt nicht, weil Paneel-Augenbraue, Ortspfad und „Stand“ geteilte Primitive sind (Präzedenz: LFH-618, Befund 8) | LFH-643, LFH-661 (app-weit) |
...
| 12 | **Kein Sprung unter dem Cursor** | **erfüllt** (LFH-647, Stand `abdd43d8`) | Fremde Neuzugänge (fremd begonnene Schicht, Folgeschicht eines fremden Vollzugs, eine fremd zurückgenommene Schicht) warten hinter dem Sammelbanner, eigene stehen sofort (`abloesung/zufluss.ts`). Die Schleuse hängt nicht am Fokus, sie gilt immer. Das Banner steht in der ohnehin gerenderten Werkzeugzeile und ändert deren Höhe nicht. **Browser:** `e2e/abloesung-zufluss.spec.ts`, die fremde Schicht wird **vor** den gezeigten einsortiert. Werkzeugzeile und Oberkante der obersten Karte vor und nach dem Banner: Fükw 1366 px `kompakt` 32 → 32 px, y 158 → 158 · Tablet 1024 px `handschuh` 74 → 74, y 406 → 406 · mobil 390 px `komfortabel` 50 → 50, y 310 → 310; zwei Karten bis zum Klick, danach drei mit der neuen oben; kein waagerechter Überlauf. Mutationsprobe im Browser: Banner ohne `paddingBlock: 0` → Zeile 32 → 38 / 74 → 88 / 50 → 60 px, alle drei rot. **Vitest:** `AbloesungPage.test.tsx` › „Live-Zufluss (LFH-647)" (fremd zurückgehalten, eigen sofort, Rennen Live-Ereignis vor eigener Antwort, Änderung an vorhandener Karte sofort, Banner in der Werkzeugzeile, Ansichtswechsel gibt frei); `abloesung/zufluss.test.ts`. Mutationsproben: `teileZufluss` → „alles sichtbar" färbt 4 Seitentests rot, ohne Vormerkung der eigenen Id 2. **Benannter Rest, bewusst so:** Entfallene fallen sofort weg (ein fremder Vollzug nimmt eine Karte heraus, die Karten darunter rücken nach), wie in `Datensicht` und der ETB-Zeitachse, denn eine nicht mehr vorhandene Karte kann man nicht zeigen. Eine fremde Änderung von Rhythmus oder Beginn ordnet vorhandene Karten nach Fälligkeit um (→ LFH-660) | — |
...
**Bilanz:** 11 erfüllt · 2 offen · 2 nicht anwendbar. Kriterium 5 ist seit LFH-646 gemessen; offen bleibt es wegen LFH-643/LFH-661. Kriterium 12 ist mit LFH-647 erfüllt.
...
| 5 | Kontrast | **offen** | Gemessen, nicht geerbt: `e2e/abloesung-kontrast.spec.ts` öffnet „Schicht beginnen“ und den Vollzug und misst jeden Text im Dialog. Tag ohne Ausnahme Minimum 17,08, Nacht 6,19. Ausnahmen wie in Tabelle 1: Feldhilfe und Platzhalter (`schwach`, Tag 5,89, Nacht 4,81) → LFH-643, Absende-Knopf (Weiß auf `bedien`, Tag 6,59) → LFH-661. „Ablöser planen“ und „Rhythmus“ laufen über dieselbe `ErfassungsModal`-Hülle mit denselben Feldarten **[abgeleitet]**. Offen wie in Tabelle 1 allein wegen der zwei app-weiten Rollen | LFH-643, LFH-661 (app-weit) |
