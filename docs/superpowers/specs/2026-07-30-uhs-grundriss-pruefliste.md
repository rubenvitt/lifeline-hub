...
| 4 · Kritische Aktion hat eine zweite Handlung | **erfüllt, und der Rückweg ist jetzt vollständig** | Die Zuweisung bleibt umkehrbar — aber unter `lg` hing die Umkehrung **allein am Drag auf `drop-inbox`**, und den gibt es im Reiter-Zweig nicht mehr (Quelle und Ziel liegen in verschiedenen Reitern). Ohne Nacharbeit hätte der Umbruch eine Bewegung **genommen**. „Zurück in den Wartebereich" im Platzaktionen-Menü ist diese Nacharbeit (`Grundriss.tsx`, nur am belegten Platz). **BEIDE Richtungen, seit dem Abschluss-Review:** dieselbe Rechnung war für `drop-transport` zunächst nicht gestellt — `onDragEnd` nimmt `kind: 'transport'` von JEDER Person entgegen, auch aus dem Wartebereich und aus „Noch nicht aufgenommen", und das Droppable liegt unter `lg` im dritten Reiter; die direkten Knöpfe sitzen nur auf der belegten Platzkarte, `PersonDetailDrawer` ist mutationsfrei. Ersatz ist ein Auslöser „Verbleib / Entlassung erfassen — <Zeilenkennung>" an JEDER Zeile beider Wartelisten (`PersonenSpalte`), der denselben Verbleib-Dialog öffnet wie der Drag. Ohne Schreibrecht wird er gar nicht erst gerendert. Weiterhin **keine** Rückfrage: umkehrbar → kein zusätzlicher Reibungsschritt (LFH-363) |
