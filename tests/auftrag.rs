...
async fn quittieren_aendert_quittung_nicht_vollzug() {
    ...
    let (status, json) = anfrage(
        &app,
        "POST",
        &format!("/api/einsaetze/{e}/auftraege/{aid}/empfaenger/{empf}/quittieren"),
        &admin,
        None,
    )
    .await;
...
async fn vollzug_melden_erzeugt_etb_meldung_und_setzt_status() {
    ...
    let (status, json) = anfrage(
        &app,
        "POST",
        &format!("/api/einsaetze/{e}/auftraege/{aid}/vollzug"),
        &admin,
        Some(body),
    )
    .await;
    ...
    .await;
    let typen: Vec<&str> = etb
        .as_array()
        .unwrap()
        .iter()
        .filter_map(|x| x["typ"].as_str())
        .collect();
...
async fn abnehmen_vor_vollzug_ist_422() {
    ...
    let (status, _) = anfrage(
        &app,
        "POST",
        &format!("/api/einsaetze/{e}/auftraege/{aid}/abnehmen"),
        &admin,
        None,
    )
    .await;
...
async fn cross_einsatz_abnehmen_ist_404() {
    ...
    let (_, a) = anfrage(
        &app,
        "POST",
        &format!("/api/einsaetze/{a_einsatz}/auftraege"),
        &admin,
        Some(&body_mit_funktion("X", "EA1")),
    )
    .await;
    ...
    let (status, _) = anfrage(
        &app,
        "POST",
        &format!("/api/einsaetze/{b_einsatz}/auftraege/{aid_a}/abnehmen"),
        &admin,
        None,
    )
    .await;
...
async fn status_filter_trennt_offen_und_vollzogen() {
    ...
    let (status, _) = anfrage(
        &app,
        "POST",
        &format!("/api/einsaetze/{e}/auftraege/{aid1}/vollzug"),
        &admin,
        Some(r#"{"status":"vollzogen","vollzugsmeldung":"x"}"#),
    )
    .await;
    assert_eq!(status, StatusCode::OK);
    ...
    let (status, json) = anfrage(
        &app,
        "GET",
        &format!("/api/einsaetze/{e}/auftraege?status=vollzogen"),
        &admin,
        None,
    )
    .await;
    ...
    let (status, json) = anfrage(
        &app,
        "GET",
        &format!("/api/einsaetze/{e}/auftraege?status=offen"),
        &admin,
        None,
    )
    .await;
...
async fn ungueltiger_vollzug_status_ist_400() {
    ...
    let (status, _) = anfrage(
        ...
        &format!("/api/einsaetze/{e}/auftraege/{aid}/vollzug"),
        &admin,
        Some(r#"{"status":"quatsch","vollzugsmeldung":"x"}"#),
    )
    .await;
    assert_eq!(status, StatusCode::BAD_REQUEST);
}
...
async fn abnehmen_nach_vollzug_ist_erfolgreich() {
    ...
    let (status, _) = anfrage(
        ...
        &format!("/api/einsaetze/{e}/auftraege/{aid}/vollzug"),
        &admin,
        Some(r#"{"status":"vollzogen","vollzugsmeldung":"fertig"}"#),
    )
    .await;
    assert_eq!(status, StatusCode::OK);
    let (status, json) = anfrage(
...
async fn richtung_filter_trennt_intern_extern() {
    ...
    anfrage(
        ...
        &format!("/api/einsaetze/{e}/auftraege"),
        &admin,
        Some(&body_mit_funktion("intern-auftrag", "EA")),
    )
    .await;
    ...
    let (status, json) = anfrage(
        &app,
        "GET",
        &format!("/api/einsaetze/{e}/auftraege?richtung=extern"),
        &admin,
        None,
    )
    .await;
    ...
    let (status, _) = anfrage(
        &app,
        "GET",
        &format!("/api/einsaetze/{e}/auftraege?richtung=quatsch"),
        &admin,
        None,
    )
    .await;
...
// --- Auto-ETB-Schalter (LFH-133) ---

#[tokio::test]
async fn auto_etb_aus_unterdrueckt_anordnung_am_http_rand() {
    let app = setup().await;
    let admin = login_cookie(&app, "admin", "startpw12").await;
    let e = einsatz_anlegen(&app, &admin).await;
    let (s, _) = anfrage(
