...
async fn anlegen_vergibt_s_nummer_und_status_offen() {
    ...
    let (s, v) = anfrage(
        ...
        &admin,
        Some(&json!({"typ":"umweltschaden","ausmass":"gross","ort":"Hauptstr. 17"})),
    )
    .await;
    assert_eq!(s, StatusCode::CREATED);
...
async fn geschaedigt_fk_auf_storniert_person_bleibt_zulaessig() {
    ...
    assert!(
        v["geschaedigt_storniert_at"].is_string(),
        "Join zeigt storniert-Marke"
    );
}
...
async fn patch_geschaedigt_xor_effektivzustand_ist_422() {
    ...
    let (s, _) = anfrage(
        ...
        &format!("/api/einsaetze/{e}/schaeden/{sid}"),
        &admin,
        Some(&json!({"geschaedigt_person_id": p})),
    )
    .await;
    assert_eq!(s, StatusCode::UNPROCESSABLE_ENTITY);
}
...
async fn liste_filtert_nach_status_typ_ausmass() {
    ...
    schaden_anlegen(
        ...
        &admin,
        e,
        &json!({"typ":"umweltschaden","ausmass":"gross","ort":"A"}),
    )
    .await;
    schaden_anlegen(
        &app,
...
async fn lifecycle_etb_je_event_ein_eintrag_ohne_leak() {
    ...
    for i in &schaden_eintraege {
        assert!(
            !i.contains("GEHEIM_NOTIZ"),
            "Leak: Abschluss-Notiz im ETB: {i}"
        );
        assert!(
            !i.contains("R-001"),
...
async fn geschaedigt_aus_fremdem_einsatz_ist_404() {
    ...
    assert_eq!(
        ...
        "Geschädigt aus fremdem Einsatz → 404"
    );
}
...
async fn anlegen_mit_geschaedigt_einsatzkraft_ist_201_mit_name() {
    ...
    assert_eq!(
        s,
        StatusCode::CREATED,
        "Einsatzkraft als Geschädigter: {v:?}"
    );
...
async fn anlegen_mit_geschaedigt_einsatzkraft_aus_fremdem_einsatz_ist_404() {
    ...
    assert_eq!(
        s,
        StatusCode::NOT_FOUND,
        "Einsatzkraft aus fremdem Einsatz → 404"
    );
}
...
async fn anlegen_mit_geschaedigt_organisation_erzwingt_eigene_org() {
    ...
    assert!(
        v["geschaedigt_organisation_name"].is_string(),
        "Org-Name aufgelöst"
    );
...
async fn patch_geschaedigt_personal_auf_kontakt_effektivzustand_ist_422() {
    ...
    let (s, _) = anfrage(
        ...
        &format!("/api/einsaetze/{e}/schaeden/{sid}"),
        &admin,
        Some(&json!({"geschaedigt_personal_id": ep})),
    )
    .await;
    assert_eq!(
        s,
        StatusCode::UNPROCESSABLE_ENTITY,
        "Effektivzustand 2 Quellen → 422, NICHT 500"
    );
}
...
async fn patch_geschaedigt_organisation_erzwingt_eigene_org() {
    ...
    let (s, _) = anfrage(
        ...
        &format!("/api/einsaetze/{e}/schaeden/{sid}"),
        &admin,
        Some(&json!({"geschaedigt_organisation_id": 99999})),
    )
    .await;
