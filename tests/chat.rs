...
async fn senden_und_lesen() {
    ...
    let (s, m) = anfrage(
        ...
        &admin,
        Some(r#"{"inhalt":"Funktionsabstimmung S2/S3"}"#),
    )
    .await;
...
async fn kanal_eines_anderen_einsatzes_ist_nicht_erreichbar() {
    ...
    let (s, _) = anfrage(
        &app,
        "POST",
        &format!("/api/einsaetze/{einsatz_a}/chat/kanaele/{kid_b}/nachrichten"),
        &admin,
        Some(r#"{"inhalt":"falscher Einsatz"}"#),
    )
    .await;
...
async fn heraufstufen_erzeugt_etb_und_sperrt_doppelt() {
    ...
    let (_, m) = anfrage(
        ...
        &format!("/api/einsaetze/{einsatz}/chat/kanaele/{kid}/nachrichten"),
        &admin,
        Some(r#"{"inhalt":"Deich km12 instabil"}"#),
    )
    .await;
    let mid = m["id"].as_i64().unwrap();
    ...
    assert!(meldungen[0]["inhalt"]
        .as_str()
        .unwrap()
        .contains("Deich km12 instabil"));
...
async fn heraufstufen_lehnt_unzulaessige_typen_ab() {
    ...
    for typ in ["system", "berichtigung", "quatsch"] {
        let (s, _) = anfrage(
            ...
            &format!("/api/einsaetze/{einsatz}/chat/nachrichten/{mid}/heraufstufen-etb"),
            &admin,
            Some(&format!(r#"{{"typ":"{typ}"}}"#)),
        )
        .await;
        assert_eq!(
            s,
            StatusCode::BAD_REQUEST,
            "Typ {typ} muss abgelehnt werden"
        );
    }
}
...
async fn heraufstufen_zu_auftrag_erzeugt_auftrag_und_markiert_nachricht() {
    ...
    let (_, m) = anfrage(
        ...
        &format!("/api/einsaetze/{einsatz}/chat/kanaele/{kid}/nachrichten"),
        &admin,
        Some(r#"{"inhalt":"Tank 5000 anfordern"}"#),
    )
    .await;
    let mid = m["id"].as_i64().unwrap();
    ...
    assert_eq!(s, StatusCode::CREATED);
    assert!(
        hoch["auftrag_id"].is_i64(),
        "Nachricht trägt den Rückverweis auf den Auftrag"
    );
    ...
    assert!(
        arr[0]["etb_anordnung_id"].is_i64(),
        "ETB-Anordnung im selben Commit"
    );
...
async fn heraufstufen_zu_auftrag_ohne_empfaenger_wird_abgelehnt() {
    ...
    let (s, _) = anfrage(
        ...
        &format!("/api/einsaetze/{einsatz}/chat/nachrichten/{mid}/heraufstufen-auftrag"),
        &admin,
        Some(r#"{"auftrag_text":"x","empfaenger":[]}"#),
    )
    .await;
    ...
    .await;
    assert!(
        nachher.as_array().unwrap()[0]["auftrag_id"].is_null(),
        "keine Markierung bei Validierungsfehler"
    );
}
...
async fn schaden_anlegen(app: &axum::Router, einsatz: i64, cookie: &str) -> i64 {
    let (s, j) = anfrage(
        ...
        cookie,
        Some(r#"{"typ":"sachschaden","ausmass":"gering","ort":"B5 km12"}"#),
    )
    .await;
    assert_eq!(s, StatusCode::CREATED);
    j["id"].as_i64().unwrap()
...
async fn bezug_setzen_anzeigen_und_loesen() {
    ...
    let (s, n) = anfrage(
        &app,
        "PUT",
        &format!("/api/einsaetze/{einsatz}/chat/nachrichten/{mid}/bezug"),
        &admin,
        Some(&format!(r#"{{"typ":"schaden","ziel_id":{sid}}}"#)),
    )
    .await;
    assert_eq!(s, StatusCode::OK);
...
async fn bezug_auf_fremden_einsatz_abgelehnt() {
    ...
    let (s, _) = anfrage(
        &app,
        "PUT",
        &format!("/api/einsaetze/{einsatz_a}/chat/nachrichten/{mid}/bezug"),
        &admin,
        Some(&format!(r#"{{"typ":"schaden","ziel_id":{sid_b}}}"#)),
    )
    .await;
    assert_eq!(s, StatusCode::BAD_REQUEST);
}
