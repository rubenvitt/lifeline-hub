...
/// Prüft, ob ein Fahrzeug zum Einsatz gehört. `NotFound` sonst.
async fn pruefe_fahrzeug(pool: &SqlitePool, einsatz_id: i64, ef_id: i64) -> Result<(), AppError> {
    let t: Option<i64> = sqlx::query_scalar("SELECT 1 FROM einsatz_fahrzeug WHERE id = ? AND einsatz_id = ?")
        .bind(ef_id).bind(einsatz_id).fetch_optional(pool).await?;
    t.map(|_| ()).ok_or(AppError::NotFound)
}

/// Ordnet eine Personal-Dispozeile einem Fahrzeug als Besatzung zu (exklusiv: eine
/// bereits andernorts zugeordnete Kraft wechselt). `NotFound`, falls Dispozeile oder
/// Fahrzeug nicht zum Einsatz gehören. Liefert den Personen-Namen (für den ETB-Text).
pub async fn ordne_besatzung_zu(pool: &SqlitePool, einsatz_id: i64, ef_id: i64, ep_id: i64) -> Result<String, AppError> {
    pruefe_fahrzeug(pool, einsatz_id, ef_id).await?;
    ...
    sqlx::query("UPDATE einsatz_personal SET fahrzeug_id = ? WHERE id = ? AND einsatz_id = ?")
        .bind(ef_id).bind(ep_id).bind(einsatz_id).execute(pool).await?;
    Ok(name)
}

/// Gibt eine Personal-Dispozeile aus ihrem Fahrzeug frei (`fahrzeug_id = NULL`).
/// `NotFound`, falls die Kraft nicht gerade diesem Fahrzeug zugeordnet ist (die
/// WHERE-Klausel verifiziert die Ist-Zuordnung). Liefert den Personen-Namen.
pub async fn gib_besatzung_frei(pool: &SqlitePool, einsatz_id: i64, ef_id: i64, ep_id: i64) -> Result<String, AppError> {
    let name: Option<String> = sqlx::query_scalar(
        "SELECT snap_name FROM einsatz_personal WHERE id = ? AND einsatz_id = ? AND fahrzeug_id = ?",
    ).bind(ep_id).bind(einsatz_id).bind(ef_id).fetch_optional(pool).await?;
    let name = name.ok_or(AppError::NotFound)?;
...
mod tests {
    ...
    async fn person(pool: &SqlitePool, einsatz: i64, name: &str) -> i64 {
        sqlx::query_scalar("INSERT INTO einsatz_personal (einsatz_id, snap_name) VALUES (?, ?) RETURNING id")
            .bind(einsatz).bind(name).fetch_one(pool).await.unwrap()
    }

    async fn fahrzeug_von(pool: &SqlitePool, ep: i64) -> Option<i64> {
        sqlx::query_scalar("SELECT fahrzeug_id FROM einsatz_personal WHERE id = ?").bind(ep).fetch_one(pool).await.unwrap()
    }
    ...
    async fn zuordnen_wechseln_freigeben() {
        ...
        assert_eq!(ordne_besatzung_zu(&pool, einsatz, a, ep).await.unwrap(), "Anna");
        ...
        // Freigeben.
        assert_eq!(gib_besatzung_frei(&pool, einsatz, b, ep).await.unwrap(), "Anna");
    ...
    async fn freigeben_falschen_fahrzeugs_ist_notfound() {
        ...
        assert!(matches!(gib_besatzung_frei(&pool, einsatz, b, ep).await.unwrap_err(), AppError::NotFound));
        assert_eq!(fahrzeug_von(&pool, ep).await, Some(a));
    }
    ...
    async fn zuordnen_fremde_dispozeile_ist_notfound() {
        ...
        assert!(matches!(ordne_besatzung_zu(&pool, einsatz, a, fremder_ep).await.unwrap_err(), AppError::NotFound));
    }
    ...
    async fn zuordnen_fremdes_fahrzeug_ist_notfound() {
        ...
        assert!(matches!(ordne_besatzung_zu(&pool, einsatz, fremdes_fz, ep).await.unwrap_err(), AppError::NotFound));
    }
}
