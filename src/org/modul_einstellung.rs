...
pub async fn setzen(
    ...
    sqlx::query(
        "INSERT INTO org_modul_einstellung \
            (org_id, modul_key, benoetigte_rolle, geaendert_at) \
         VALUES (?, ?, ?, datetime('now')) \
...
mod tests {
    ...
    /// Legt Org(1) an; kein Benutzer nötig (kein erfasser_id).
    ...
    async fn setzen_dann_laden_enthaelt_eintrag() {
        ...
        setzen(&pool, 1, "etb", Some("fuehrungskraft"))
            .await
            .unwrap();
    ...
    async fn setzen_none_setzt_null_zeile_bleibt() {
        ...
        setzen(&pool, 1, "etb", Some("fuehrungskraft"))
            .await
            .unwrap();
        setzen(&pool, 1, "etb", None).await.unwrap();
    ...
    async fn org_isolation_fremde_org_beeinflusst_map_nicht() {
        ...
        setzen(&pool, 1, "chat", Some("admin")).await.unwrap();
