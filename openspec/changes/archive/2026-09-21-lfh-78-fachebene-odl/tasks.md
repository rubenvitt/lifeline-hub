...
- [x] 6.3 Voller Gate-Lauf `./scripts/check-all.sh` grün (fmt/Prettier, Lint, Typ-Codegen, `cargo test`, Vitest, e2e); verifiziert durch Exit-Code 0 — auf `a13db350`: Schritte 1–6 im Gesamtlauf, e2e 169/169 nach Neubau des Binarys (das geteilte Cargo-Target trug ein Binary einer Parallel-Session mit fremdem `frontend/dist`), 8/9 im Bündel `schnell`
- [x] 6.4 Browser-Sichtprüfung auf der Lagekarte (Dev-Stack): Ebene zuschalten, Sonden erscheinen, Inspector zeigt Wert + Hinweis, Attribution unten rechts, Hell/Dunkel; verifiziert durch Screenshot — über die Playwright-Harness (Anmeldung wie die Bestands-Specs, ODL-Antwort per `page.route` vorgegeben), Hell und Dunkel; die stark erhöhte Sonde liegt trotz erster Position in der Antwort oben (`circle-sort-key` live im Stil)
