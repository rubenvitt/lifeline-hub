...
| 7 | Eine Farbe = eine Bedeutung | **erfüllt** | Sichtungsfarben kommen ausschließlich aus `tokens.ts` (`sichtungsfarben`). SK IV/blau ist die benannte Ausnahme aus LFH-455. Das Farbfeld der Zeile ist bewusst `text2`, nicht `bedien`: Blau bedient. Ein Personen-Cluster ist mit der dringlichsten Sichtung gefüllt und trägt deren Kürzel. Das ist dieselbe Aggregation wie beim Sichtungs-Donut aus LFH-650, das frühere Rosé-Literal ist entfallen. |
