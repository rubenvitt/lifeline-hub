...
- [x] 2.4 (aus dem Review) Belegte Texte bei der Vergabe überspringen: ein früher von Hand gesetzter Wert im neuen Muster (`E-2026-0005`) trägt keine Zahlen und zählt im MAX nicht mit. Ohne Ausweichen schlüge der Text-Index an, bei jedem weiteren Versuch wieder. Verifikation: `anlegen_ueberspringt_handbelegten_nummerntext` (rot vor dem Fix, grün danach), dazu die unbekannte Zeitzone im Test der Vergabe.
...
- [x] 5.2 `pages/einstellungen/orgEinstellungenForm.ts` + `EinsatzDefaults.tsx`: Feld „Präfix Einsatznummer“ (Platzhalter `E-`) aufnehmen, in `zuUpdate` **und** im Einsatz-Normalizer. Verifikation: der Form-Test zeigt, dass ein Speichern der **Anzeige**-Sektion den Bestandswert von `einsatz_nummer_praefix` mitschickt, und ein Komponententest, dass das Feld beim Speichern der Einsatz-Defaults im PUT steht.
...
- [x] 5.3 Bestehende Frontend-Tests mit `einsatznummer_intern`-Fixtures laufen lassen (Kopfzeile, Kachel, Lagekarte …). Verifikation: die volle Vitest-Suite ist grün.

## 6. Abschluss

- [x] 6.1 `./scripts/check-all.sh` komplett grün (fmt, lint, Codegen/tsc, cargo test, Vitest, deps, e2e).
