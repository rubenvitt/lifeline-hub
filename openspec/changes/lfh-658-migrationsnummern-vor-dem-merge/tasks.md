...
- [x] 1.1 Charakterisierungstest in `src/db.rs`: Zwei Hand-`Migrator` laufen gegen eine
...
- [x] 2.1 `scripts/check-migrationen.test.sh` gegen echte Git-Repos im Temp-Verzeichnis
...
- [x] 2.2 `scripts/check-migrationen.sh` umsetzen (Regeln und `--umnummerieren` wie in
  design.md D4), bis 2.1 grün ist. Danach gegen `origin/alpha` auf diesem Branch ausführen
  und das Ergebnis grün sehen.
- [x] 2.3 Mutationsprobe von Hand: Regel „> max(B)“ auf „∉ B“ abschwächen, dann muss der
