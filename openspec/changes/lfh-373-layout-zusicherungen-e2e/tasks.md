...
- [ ] 5.1 Roter Nachweis in `fokus-verdeckung.spec.ts`: Lagekarte bei 390 × 844 (Leiste aus- und eingeblendet) und 1024 × 768 in `handschuh`, Zeitachse ausgeklappt, `pruefeFokusVerdeckung` mit `zusatzKandidaten` der Karte und `region: '[data-lfh="kartenspalte"]'`. Dazu Box-Schnitt Knopfblock gegen jedes Fußband und ein echter Klick auf jeden Kartenknopf (`toBeVisible` belegt keine Klickbarkeit). Verifiziert durch einen roten Lauf vor dem Fix (Vorab-Messung: Herauszoomen/Norden/Messen 100 % unter dem Band)
- [ ] 5.2 `fussStil` wird Funktion der Knopfkante, der Fuß endet vor der Knopfspalte (design.md D6), in `KartenFuss` und jedem weiteren Fuß-Aufrufer. Verifiziert dadurch, dass 5.1 grün wird, durch Vitest auf die reine Funktion (`right` = 2 × Abstand + Kante für kompakt und handschuh) und das unveränderte Grün von `lagekarte-smoke.spec.ts` (LFH-355-Stapelung)
- [ ] 5.3 Personenkarte (`/personen?ansicht=karte`, LFH-613 4·13): derselbe Lauf mit den Kartenkandidaten bei 390 × 844 und 1366 × 768 in `handschuh`. Verifiziert durch den grünen Test, bei rotem Ergebnis nach dem Fix aus 5.2
- [ ] 5.4 Personenliste (LFH-613 1·13): Tab-Durchlauf im Muster des KatalogTabelle-Tests auf der Personenroute mit gesäten Personen (`POST …/personen`), 390 × 400 und 1366 × 520, Bildlaufreserve als Vorbedingung. Verifiziert durch den grünen Test oder, falls rot, durch einen Fix nach dem Muster aus 4.2
