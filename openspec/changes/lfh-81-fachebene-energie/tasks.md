...
- [ ] 1.1 `Bbox`: einen Cache-Schlüssel mit Präfix bauen. `kritis:` bleibt byte-gleich.
  Belegt durch einen Unit-Test auf beide Präfixe, einer davon mit handgeschriebenem
  `kritis:`-Literal.
- [ ] 1.2 Anlagenart aus MaStR-`EnergietraegerName` und OSM-`plant:source` (erster Wert
  ...
  der `sonstige` ergibt.
- [ ] 1.3 Leistungsleser für `plant:output:electricity` (MW, GW, kW, mit oder ohne
...
- [ ] 1.4 `normalisiere_energie_osm`: Overpass-JSON wird zu Punkten mit Rauschfilter
...
- [ ] 1.5 `normalisiere_energie_mastr`: MaStR-`Data` wird zu Punkten. Eine fehlende
...
- [ ] 1.6 Zusammenführung per Nähe und Anlagenart (nächste gleichartige OSM-Anlage im
  ...
  Ausschnitt“.
- [ ] 1.7 Die Quellennennung aus den beitragenden Teilen bauen. Belegt durch Tests für
  beide Quellen, nur OSM, nur MaStR und keine Quelle.

## 2. Backend: Abruf und Route

- [ ] 2.1 MaStR-Abruf mit Upstream-Filter (design.md, Entscheidung 2), eigenem
...
- [ ] 2.2 `fetch_energie`: beide Teile über `liefere_mit_swr` mit den Schlüsseln
...
- [ ] 2.3 Match-Arm `"energie"` mit Pflicht-bbox in `routes/karte.rs`. Belegt durch
...
- [ ] 2.4 Den Zuordnungsradius messen: gegen die Live-Endpunkte an einer Stichprobe von
...
- [ ] 2.5 Bei laufendem Backend ein Live-Abruf `GET /api/karte/fachebenen/energie?bbox=`
  im Ruhrgebiet. Belegt dadurch, dass Scholven und GuD Herne erscheinen und keine
  PV-Kleinanlage.

## 3. Frontend: bbox-Pfad verallgemeinern

- [ ] 3.1 `KRITIS_MIN_ZOOM` wird zu `BBOX_MIN_ZOOM`. `Kartenflaeche` meldet die bbox, sobald
...
- [ ] 3.2 `useFachebenen`: ein gemeinsamer `viewportBbox`, eine Akkumulation je bbox-Ebene
...
- [ ] 4.1 `'energie'` kommt in `FachebeneQuelle`, `FACHEBENEN`, `fachebeneKeys()` und
...
- [ ] 4.2 Ein siebter literaler `useQueries`-Eintrag mit `placeholderData: keepPreviousData`
  ...
  akkumuliert.
- [ ] 4.3 `FachebenenInspector`: ein Zweig `EnergieInhalt` mit Titel, Anlagenart, Leistung
...
- [ ] 4.4 Persistenz: `defaultFachebenenSichtbar` und `leseFachebenen` bekommen `energie`.
...
- [ ] 5.1 `docs/fachebenen-quellen.md`: eine Zeile für `energie` (beide Endpunkte, TTL,
  ...
  Gesamtexport). Dazu ein Eintrag in `CHANGELOG.md`. Belegt durch Sichtprüfung im Diff.
