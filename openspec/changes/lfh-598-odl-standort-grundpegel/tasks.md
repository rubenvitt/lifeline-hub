...
- [x] 1.4 Reine Funktion für die Zeitpunkt-Liste (28 volle UTC-Stunden im 6-h-Raster rückwärts, innerhalb von 7 Tagen) und den daraus gebauten CQL-Filter; Test mit fester Uhrzeit prüft Anzahl, Raster, ältesten/jüngsten Zeitpunkt und URL-Kodierung
...
- [x] 2.3 Wire-Pin: `bewertung`-Wörter `standort`/`absolut` in `odl_tests` gepinnt, Stufenwörter-Pin unverändert grün
...
- [x] 3.2 `grundpegel_antwort(roh)` rein: ohne `features`-Liste → `None` (Formatbruch schreibt nichts); Test nach Muster `odl_antwort_tests`
