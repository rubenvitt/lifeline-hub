...
describe('NachrichtenStrom', () => {
  ...
  it('zeigt Tombstone für gelöschte Nachrichten ohne Aktionen', () => {
    renderMitProviders(
      <NachrichtenStrom
        nachrichten={[nachricht({ inhalt: null, geloescht_at: '2026-06-10 10:05:00' })]}
        eigeneBenutzerId={1}
        darfSchreiben
        onBearbeiten={vi.fn()}
        onLoeschen={vi.fn()}
  ...
  it('zeigt ETB-Badge bei heraufgestufter Nachricht', () => {
    renderMitProviders(
      <NachrichtenStrom
        nachrichten={[nachricht({ etb_eintrag_id: 42 })]}
        eigeneBenutzerId={1}
        darfSchreiben
        onBearbeiten={vi.fn()}
        onLoeschen={vi.fn()}
  ...
  it('zeigt Auftrag-Badge bei zu Auftrag heraufgestufter Nachricht und blendet „Zu Auftrag" aus', async () => {
    renderMitProviders(
      <NachrichtenStrom
        nachrichten={[nachricht({ auftrag_id: 7 })]}
        eigeneBenutzerId={1}
        darfSchreiben
        onBearbeiten={vi.fn()}
        onLoeschen={vi.fn()}
  ...
  it('blendet Bearbeiten/Löschen bei fehlendem Schreibrecht aus (eigene Nachricht)', () => {
    renderMitProviders(
      <NachrichtenStrom
        nachrichten={[nachricht({ autor_id: 1 })]}
        eigeneBenutzerId={1}
        darfSchreiben={false}
        onBearbeiten={vi.fn()}
        onLoeschen={vi.fn()}
  ...
  it('Aktionen nur an eigenen Nachrichten; Heraufstufen löst Callback aus', async () => {
    ...
    renderMitProviders(
      <NachrichtenStrom
        nachrichten={[
          nachricht({ id: 1, autor_id: 1 }),
          nachricht({ id: 2, autor_id: 99, inhalt: 'fremd' }),
        ]}
        eigeneBenutzerId={1}
        darfSchreiben
        onBearbeiten={vi.fn()}
  ...
  it('zeigt den Sachbezug als Tag mit aufgelöstem Label', () => {
    renderMitProviders(
      <NachrichtenStrom
        nachrichten={[nachricht({ bezug_typ: 'schaden', bezug_id: 3 })]}
        eigeneBenutzerId={1}
        darfSchreiben
        bezugLabel={(typ, id) => `${typ} S-00${id}`}
        onBezugSetzen={vi.fn()}
        onBezugLoeschen={vi.fn()}
        onBearbeiten={vi.fn()}
        onLoeschen={vi.fn()}
        onHeraufstufen={vi.fn()}
        onHeraufstufenAuftrag={vi.fn()}
    ...
    renderMitProviders(
      <NachrichtenStrom
        nachrichten={[
          nachricht({
            inhalt: null,
            geloescht_at: '2026-06-10 10:05:00',
            bezug_typ: 'schaden',
            bezug_id: 3,
          }),
        ]}
  ...
  it('„Bezug" löst onBezugSetzen aus; Label wechselt zu „Bezug ändern" wenn gesetzt', async () => {
    ...
    const { rerender } = renderMitProviders(
      <NachrichtenStrom
        nachrichten={[nachricht({ id: 9 })]}
        eigeneBenutzerId={1}
        darfSchreiben
        bezugLabel={() => 'x'}
        onBezugSetzen={onBezugSetzen}
        onBezugLoeschen={vi.fn()}
        onBearbeiten={vi.fn()}
        onLoeschen={vi.fn()}
        onHeraufstufen={vi.fn()}
    ...
    rerender(
      <NachrichtenStrom
        nachrichten={[nachricht({ id: 9, bezug_typ: 'meldung', bezug_id: 2 })]}
        eigeneBenutzerId={1}
        darfSchreiben
        bezugLabel={() => 'x'}
        onBezugSetzen={onBezugSetzen}
  ...
  it('öffnet bei Klick auf den Bezug-Tag ein Popover mit Kurzinfo', async () => {
    renderMitProviders(
      <NachrichtenStrom
        ...
        bezugInfo={() => ({
          titel: 'S-003 · sachschaden',
          zeilen: ['Ausmaß: gering', 'Ort: B5 km12'],
        })}
        onBezugSetzen={vi.fn()}
        onBezugLoeschen={vi.fn()}
        onBearbeiten={vi.fn()}
  ...
  it('Bezug-Tag ist schließbar und löst onBezugLoeschen aus', async () => {
    ...
    const { container } = renderMitProviders(
      <NachrichtenStrom
        nachrichten={[nachricht({ id: 4, bezug_typ: 'auftrag', bezug_id: 8 })]}
        eigeneBenutzerId={1}
        darfSchreiben
        bezugLabel={() => 'Auftrag-Label'}
        onBezugSetzen={vi.fn()}
        onBezugLoeschen={onBezugLoeschen}
        onBearbeiten={vi.fn()}
        onLoeschen={vi.fn()}
        onHeraufstufen={vi.fn()}
        onHeraufstufenAuftrag={vi.fn()}
  ...
  it('zeigt Anhänge als Download-Link mit Dateiname, href und Größe', () => {
    ...
    renderMitProviders(
      <NachrichtenStrom
        nachrichten={[nachricht({ anhaenge: [anhang] })]}
        eigeneBenutzerId={1}
        darfSchreiben
        onBearbeiten={vi.fn()}
        onLoeschen={vi.fn()}
...
describe('NachrichtenStrom — Pille „n neue Nachrichten"', () => {
  ...
  function aufbau(nachrichten: ChatNachricht[]) {
    ...
    const { rerender } = renderMitProviders(
      <NachrichtenStrom nachrichten={nachrichten} {...gemeinsam} />,
    );
