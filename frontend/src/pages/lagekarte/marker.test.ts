...
describe('baueMarker', () => {
  it('trennt verortete von nicht-verorteten Objekten', () => {
    ...
    const { verortet, nichtVerortet } = baueMarker(
      ...
      [
        ...
        uhs({ id: 6, bezeichnung: 'PA', lat: null, lon: null }),
      ],
      [
        schaden({ id: 9, registrier_nr: 3, lat: 51, lon: 7 }),
        schaden({ id: 10, registrier_nr: 4 }),
      ],
      token,
    );
  ...
  it('färbt Schaden-Marker nach Ausmaß', () => {
    const { verortet } = baueMarker(
      undefined,
      [],
      [schaden({ id: 1, ausmass: 'katastrophal', lat: 51, lon: 7 })],
      token,
    );
...
describe('baueFreiesZeichenTz (accepts-Gating aus taktische-zeichen-core-Katalog)', () => {
  ...
  it('gated auch die Farbe: nur farb-akzeptierende Grundzeichen tragen tz.farbe', () => {
    ...
    expect(
      baueFreiesZeichenTz(freiesZeichen({ grundzeichen: 'gefahr', farbe: '#ff0000' })).farbe,
    ).toBe('#ff0000');
    expect(
      baueFreiesZeichenTz(freiesZeichen({ grundzeichen: 'taktische-formation', farbe: '#ff0000' }))
        .farbe,
    ).toBeUndefined();
  });
});
