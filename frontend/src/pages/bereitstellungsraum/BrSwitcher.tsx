...
export default function BrSwitcher({ einsatzId, aktuellerBr }: { einsatzId: number; aktuellerBr: Bereitstellungsraum }) {
  const navigate = useNavigate();
  const [anlegen, setAnlegen] = useState(false);
  const { data: liste = [] } = useQuery({ queryKey: einsatzKeys.br(einsatzId), queryFn: () => listeBr(einsatzId) });
  const sichtbar = liste.filter((b) => !b.storniert_at);
  return (
    <>
      <EinstiegSwitcher
        aktuell={aktuellerBr}
        eintraege={sichtbar.map((b) => ({ id: b.id, bezeichnung: b.bezeichnung, darstellung: brStatus[b.status], rang: STATUS_RANG[b.status] }))}
