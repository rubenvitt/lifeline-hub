...
const angelegt = {
  ...
  antreff_ort: 'Sammelstelle',
  melder_kontakt: null,
  notiz: null,
  aktuelle_sichtung: 'sk2',
  aktuelle_sichtung_at: '2026-05-27 09:05:00',
  erfasst_at: '2026-05-27 09:05:00',
  erfasst_von: 1,
  geaendert_at: '2026-05-27 09:05:00',
  geaendert_von: 1,
  storniert_at: null,
};
...
describe('AufnahmePage — UHS-Auftrag (LFH-341 · C6, Befund H38)', () => {
  ...
  it('erfasst Person und UHS-Eintritt mit genau einem Schreibrequest', async () => {
      ...
      [
        http.post('/api/einsaetze/1/personen', async ({ request }) => {
          ...
          return HttpResponse.json(
            { ...angelegt, aktuelle_uhs_id: 7, aktueller_platz_id: null },
            { status: 201 },
          );
        }),
      ],
    ...
    await userEvent.click(screen.getByRole('button', { name: 'Speichern und nächste' }));
    expect(
      await screen.findByText('Erfasst als R-047 · SK II · im Wartebereich'),
    ).toBeInTheDocument();
  ...
  it.each(['', '?uhs=kaputt'])(
    'sendet ohne gültigen UHS-Auftrag kein uhs_id (%s)',
    async (query) => {
      let daten: Record<string, unknown> = {};
      render(
        einsatzAktiv,
  ...
  it.each(['Erfassen', 'Speichern und nächste'])(
    ...
    async (aktion) => {
      ...
      expect(queue[0].aktion).toMatchObject({
        art: 'person',
        daten: { uhs_id: 7, client_id: expect.any(String) },
      });
    ...
    render(
      ...
      [
        http.post('/api/einsaetze/1/personen', () =>
          HttpResponse.json({ error: 'UHS ist nicht aktiv' }, { status: 422 }),
        ),
      ],
      '/einsaetze/1/personen/aufnahme?uhs=7',
    );
  ...
  it('liest den Wartebereich aus der Antwort, auch beim Replay nach einem Austritt', async () => {
    render(
      ...
      [
        http.post('/api/einsaetze/1/personen', () =>
          HttpResponse.json(
            { ...angelegt, aktuelle_uhs_id: null, aktueller_platz_id: null },
            { status: 201 },
          ),
        ),
      ],
