...
const meldebildSpalten = spaltenFuer<MeldebildZeile>()([
  {
    ...
    immerSichtbar: true,
    render: (_t, z) => (
      <span style={{ fontWeight: z.art === 'abschnitt' ? 600 : 400 }}>{z.bezeichnung}</span>
    ),
  },
  ...
  { title: 'Typ / Rolle', dataIndex: 'detail', key: 'detail', abBreite: 'md' },
  {
    ...
    width: 130,
    render: (_t, z) =>
      z.art === 'mittel' && z.mittelArt !== 'person' ? null : staerkeText(z.staerke),
  },
  {
    title: 'Personal',
    key: 'personal',
  ...
  },
  {
    ...
    render: (_t, z) => {
      ...
      if (z.mittelArt === 'material') {
        const text = [z.statusLabel, z.menge != null ? `×${z.menge}` : null]
          .filter(Boolean)
          .join(' · ');
      ...
      >
              ...
              <Space size="small" wrap style={{ marginBlockStart: token.marginXXS }}>
                {MAT_STATUS_ANZEIGE.filter(({ key }) => v.materialStatus[key] > 0).map(
                  ({ key, label, rolle }) => (
                    <span
                      key={key}
                      style={rolle ? { color: rollenFarbe(rolle, token) } : undefined}
                    >
        ...
        <Datensicht
          ...
          karte={{
            art: 'plan',
            titel: { spalte: 'bez' },
            sekundaer: ['detail', 'staerke', 'status'],
          }}
        />
      </EinsatzSeite>
    </div>
