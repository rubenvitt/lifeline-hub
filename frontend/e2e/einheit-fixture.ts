  ...
  for (const [modul, zuordnung, adhoc] of [
    ...
    for (const frei of [false, true]) {
      const id = await post(`${basis}/${modul}`, {
        adhoc: Object.fromEntries(Object.entries(adhoc).map(([k, v]) => [k, `${v}${frei ? ' frei' : ''}`])),
        ...(modul === 'material' ? { menge: 3 } : {}),
      });
      if (!frei) {
        const antwort = await page.request.put(`${basis}/einheiten/${einheitId}/${zuordnung}/${id}`);
...
export function zuordnungsKarte(page: Page, titel: string): Locator {
  return page.getByRole('main').locator('.ant-card').filter({
...
export function kopfFelder(page: Page): { name: string; huelle: Locator; fokus: Locator }[] {
  ...
  for (const name of ['Name', 'Typ', 'Abschnitt', 'Über-Einheit', 'Sprechgruppen', 'Kommunikationsmittel', 'Erreichbarkeit / Nummer', 'Bemerkung']) {
    ...
    const huelle = gruppe.locator('.ant-select, .ant-input-affix-wrapper, textarea, input.ant-input:not(.ant-input-affix-wrapper input)');
  ...
  for (const name of ['Führer', 'Unterführer', 'Mannschaft']) {
    ...
    const huelle = form.locator('.ant-input-number').filter({ has: page.getByRole('spinbutton', { name, exact: true }) });
    ergebnis.push({ name, huelle, fokus });
  }
  return ergebnis;
}
...
export async function detailBereit(page: Page) {
  ...
  for (const titel of ['Personal', 'Fahrzeuge', 'Material']) {
    await expect(zuordnungsKarte(page, titel).getByRole('button', { name: 'Entfernen', exact: true })).toHaveCount(1);
  }
}
